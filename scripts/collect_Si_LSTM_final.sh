...
n_known_expect = ues * 7  # 依你的定案：ues=1 對應 7 UE，ues=2 對應 14，以此類推
...
echo
echo "############################################################"
echo "# 階段 0：對 ues=2 做一次短模擬校準（用實測速度決定後續策略）"
echo "############################################################"
# 用 20 秒的短模擬校準 ues=2 的真實速度。
# 給的 timeout 用「ues=1 速度的 3 倍」當保守上限，若 20 秒都跑不完，
# 代表 ues=2 太貴，直接放棄 ues=2/3，全程用 ues=1。
CALIB_SIMTIME=20
CALIB_TIMEOUT=$(( KNOWN_RATE_UES1_SEC_PER_SIMSEC * 3 * CALIB_SIMTIME + 300 ))
echo "校準模擬：ues=2, simTime=${CALIB_SIMTIME}s, timeout=$(fmt_hms ${CALIB_TIMEOUT})"

MAIN_UES=1
RATE=${KNOWN_RATE_UES1_SEC_PER_SIMSEC}
if [ $(remaining_sec) -gt $((CALIB_TIMEOUT + 3600)) ]; then
  if run_sim 2 ${CALIB_SIMTIME} 9999 "${PROBE_DIR}" ${CALIB_TIMEOUT}; then
    MEASURED_RATE=$(( LAST_ELAPSED / CALIB_SIMTIME ))
    echo "  ues=2 校準成功：實測速度 ${MEASURED_RATE} 秒真實/秒模擬"\
         "（ues=1 為 ${KNOWN_RATE_UES1_SEC_PER_SIMSEC}）"
    MAIN_UES=2
    RATE=${MEASURED_RATE}
  else
    echo "  ues=2 校準失敗（20 秒模擬都跑不完）→ 放棄 ues=2/3，全程使用 ues=1"
    MAIN_UES=1
    RATE=${KNOWN_RATE_UES1_SEC_PER_SIMSEC}
  fi
else
  echo "  剩餘預算不足以安全校準 ues=2（需要 $(fmt_hms $((CALIB_TIMEOUT+3600)))，"\
       "剩 $(fmt_hms $(remaining_sec))）→ 跳過校準，直接用 ues=1"
fi
...
# ---- 依剩餘預算動態決定 simTime：優先 300s，塞不下就降到 180s，再不行降到 120s ----
choose_simtime() {
  local rate=$1 budget=$2
  for st in 300 180 120; do
    local t; t=$(compute_timeout "${rate}" "${st}")
    # 至少要能塞下 2 輪才選這個 simTime，否則降級
    if [ $(( t * 2 )) -le "${budget}" ]; then
      echo "${st}"; return
    fi
  done
  echo "60"   # 保底：連 2 輪 120s 都塞不下，退到最短的 60s（等同 baseline 長度）
}
...
if [ "${SIMTIME}" -le 60 ]; then
  echo "⚠️  剩餘時間只夠跑 baseline 長度（60s），無法產出比現有資料更長的序列。"
  echo "    建議：先確認 8-9 小時預算是否從現在算起、機器是否有其他負載，再重新排程。"
fi

SEED_POOL=(4200 4201 4202 4203 4204 4205 4206)
COMPLETED=()
